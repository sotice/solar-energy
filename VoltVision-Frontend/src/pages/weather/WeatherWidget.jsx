...
//   if (isLoading) {
//     return (
//       <div className="h-full rounded-2xl border border-gray-200  p-6 shadow-sm flex flex-col gap-6">
...
//         {/* Day/Night Badge */}
//         <div className={`px-3 py-1 rounded-full text-xs font-semibold border ${isDay ? "bg-sky-50 text-sky-700 border-sky-100" : " text-slate-300 border-slate-700"}`}>
...
//     return (
//         <div className="flex flex-col items-center justify-center p-3 rounded-xl border border-gray-50 bg-gray-50/30 hover:bg-blue-10 hover:shadow-sm hover:border-gray-200 transition-all duration-200">
//             <div className={`p-1.5 rounded-lg ${bg} ${color} mb-2`}>
//                 <Icon className="w-4 h-4" />
//             </div>
//             <span className="text-sm font-bold ">{value}</span>
//             <span className="text-[10px] font-medium  uppercase tracking-wide text-center">{label}</span>
//         </div>
//     );
// }

import { CloudSun, Sun, Cloud, Wind, Zap, Activity, Battery, Thermometer } from "lucide-react";
import { useGetWeatherQuery } from "@/lib/redux/query";
...
export default function WeatherWidget({ solarUnit }) {
  ...
  if (isLoading) {
    return (
      <div className="h-full rounded-2xl border border-base-200 bg-base-100 p-6 shadow-sm flex flex-col gap-6">
        <div className="flex justify-between items-center">
          <div className="h-6 w-32 bg-base-300 rounded animate-pulse" />
          <div className="h-8 w-24 bg-base-300 rounded-full animate-pulse" />
        </div>
        <div className="grid grid-cols-2 gap-4">
          <div className="h-24 bg-base-200 rounded-xl animate-pulse" />
          <div className="h-24 bg-base-200 rounded-xl animate-pulse" />
        </div>
        <div className="grid grid-cols-3 gap-4 mt-auto">
          <div className="h-16 bg-base-200 rounded-xl animate-pulse" />
          <div className="h-16 bg-base-200 rounded-xl animate-pulse" />
          <div className="h-16 bg-base-200 rounded-xl animate-pulse" />
        </div>
      </div>
  ...
  // Dynamic Theme Logic (Using standard warning/info colors from daisyUI)
  let WeatherIcon = Sun;
  // Default: Sunny/Good (Success colors)
  let themeColor = "text-warning"; 
  let themeBg = "bg-warning/10";
  let statusText = "Excellent Conditions";
  
  if (clouds > 20) { 
      WeatherIcon = CloudSun; 
      themeColor = "text-warning"; 
      themeBg = "bg-warning/10"; 
      statusText = "Good Output"; 
  }
  if (clouds > 60) { 
      WeatherIcon = Cloud; 
      themeColor = "text-neutral-content"; 
      themeBg = "bg-neutral/10"; 
  ...
  return (
    <div className="h-full rounded-2xl border border-base-200 bg-base-100 shadow-sm overflow-hidden flex flex-col hover:shadow-md transition-shadow duration-300">
      
      {/* --- HEADER --- */}
      <div className="px-6 py-5 border-b border-base-200 flex flex-row items-center justify-between">
        <div className="flex items-center gap-3">
          <div className={`p-2.5 rounded-xl ${themeBg}`}>
            <WeatherIcon className={`w-5 h-5 ${themeColor}`} />
          </div>
          <div>
            <h3 className="text-sm font-bold leading-none text-base-content">Environmental Context</h3>
            <p className="text-xs mt-1 text-base-content/60">{statusText}</p>
          </div>
        </div>
        ...
        <div className={`px-3 py-1 rounded-full text-xs font-semibold border ${
            ...
            : "bg-base-300 text-base-content/50 border-base-content/10"
        }`}>
           {isDay ? "Daytime" : "Nighttime"}
        </div>
      </div>
      ...
      <div className="p-6 flex flex-col gap-6 flex-1">
        ...
        <div className="grid grid-cols-2 gap-4">
            ...
            {/* 1. Ambient Temp */}
            <div className="relative overflow-hidden rounded-2xl bg-base-200/50 border border-base-200 p-4 group">
                <div className="flex justify-between items-start mb-2">
                    <span className="text-xs font-bold text-base-content/60 uppercase tracking-wider">Temperature</span>
                    <Thermometer className="w-4 h-4 text-error group-hover:scale-110 transition-transform" />
                </div>
                <div className="text-3xl font-bold tracking-tight text-base-content">{temp}°</div>
                <div className="text-xs mt-1 text-base-content/50">Local Ambient</div>
            </div>

            {/* 2. Real-Time Output */}
            <div className="relative overflow-hidden rounded-2xl bg-base-200/50 border border-base-200 p-4 group">
                <div className="flex justify-between items-start mb-2">
                    <span className="text-xs font-bold text-base-content/60 uppercase tracking-wider">Current Output</span>
                    <Zap className="w-4 h-4 text-warning fill-warning group-hover:scale-110 transition-transform" />
                </div>
                <div className="flex items-baseline gap-1">
                    <span className="text-3xl font-bold tracking-tight text-base-content">{realTimePower}</span>
                    <span className="text-sm font-medium text-base-content/50">kW</span>
                </div>
                {/* Efficiency Bar */}
                <div className="mt-3 w-full bg-base-300 h-1.5 rounded-full overflow-hidden">
                    <div className="h-full bg-warning rounded-full transition-all duration-1000" style={{ width: `${efficiencyPercent}%` }} />
                </div>
            </div>
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            <MiniStat 
                icon={Wind} color="text-info" bg="bg-info/10"
                label="Wind Speed" value={`${windSpeed} m/s`} 
            />
            <MiniStat 
                icon={Cloud} color="text-base-content" bg="bg-base-content/10"
                label="Cloud Cover" value={`${clouds}%`} 
            />
            <MiniStat 
                icon={Activity} color="text-success" bg="bg-success/10"
                label="Capacity" value={`${peakPower} kW`} 
            />
            <MiniStat 
                icon={Battery} color="text-primary" bg="bg-primary/10"
                label="Est. MWh" value={totalEnergy} 
            />
        </div>
...
function MiniStat({ icon: Icon, label, value, color, bg }) {
    return (
        <div className="flex flex-col items-center justify-center p-3 rounded-xl border border-base-200 bg-base-100 hover:bg-base-200/50 hover:shadow-sm transition-all duration-200">
            <div className={`p-1.5 rounded-lg ${bg} ${color} mb-2`}>
                <Icon className="w-4 h-4" />
            </div>
            <span className="text-sm font-bold text-base-content">{value}</span>
            <span className="text-[10px] font-medium uppercase tracking-wide text-center text-base-content/50">{label}</span>
        </div>
    );
}